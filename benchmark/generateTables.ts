...
import {
  generateMutantsTable,
  getTemperature,
  getTemplate,
} from "./generateMutantsTable";
...
console.log(
  `Generating tables from data in ${dataDir} and saving them in ${outputDir}`
);
...
for (const modelName of [
  ...
  for (const configuration of configurations) {
    ...
    for (const run of fs.readdirSync(configurationDir)) {
      ...
      const mutantsTablePath = path.join(
        outputDir,
        modelName,
        subDir,
        run,
        mutantsTableFileName
      );

      const costsTableFileName = `${run}-costs-table.tex`;
      const costsTablePath = path.join(
        outputDir,
        modelName,
        subDir,
        run,
      ...
      console.log(
        `  -- writing mutants table for run #${runNr} of ${modelName} with configuration ${configuration} to ${mutantsTablePath}`
      );
      fs.writeFileSync(mutantsTablePath, mutantsTable);
      console.log(
        `  -- writing costs table for run #${runNr} of ${modelName} with configuration ${configuration} to ${costsTablePath}`
      );
    ...
    const variabilityTablePath = path.join(
      outputDir,
      "variability",
      tableFileName
    );
    ...
    console.log(
      `  -- writing variability table for runs# ${runs} of ${modelName} with ${configuration} to ${variabilityTablePath}`
    );
